class LearnersProfile.UI
  constructor: (@api) ->
    ...
    @editProjectLinksModal = new Modal.App('#edit-project-links-modal', 740, 700, 640, 640)
  ...
  openProjectLinksModal: ->
    self = @
    self.editProjectLinksModal.open()
    $('body').css('overflow', 'hidden')

  closeProjectLinksModal: ->
    self = @
    self.editProjectLinksModal.close()
    $('body').css('overflow', 'auto')
  ...
  bindProjectLinksModalToOpen: ->
    self = @
    $('.edit-output-links-modal').on 'click', ->
      self.openProjectLinksModal()

  bindProjectLinksModalToClose: ->
    self = @
    $('.close-modal, .cancel-button').on 'click', ->
      self.closeProjectLinksModal()
  ...
  bindUpdateBtnToClick: (updatePersonalDetails) ->
    ...
    $('#update-personal-details').on 'click', ->
      ...
      if self.personalDetailsChanged(personalDetails)
        self.loaderUI.show()
        self.api.updatePersonalDetails(personalDetails).then(
  ...
  bindUpdateProjectLinksBtnToClick: ->
    self = @
    $('#update-project-links').on 'click', ->
      self.loaderUI.show()
      learnerLinks = {
        github_pages: $("#github-link").val()
        story_board: $("#story-board").val()
        challenge_one: $("#challenge-one-link").val()
        challenge_two: $("#challenge-two-link").val()
        challenge_three: $("#challenge-three-link").val()
        challenge_four: $("#challenge-four-link").val()
        heroku_one: $("#heroku-one").val()
        heroku_two: $("#heroku-two").val()
      }
      self.api.updateProjectLinks(learnerLinks).then(
  ...
  handleResponseMessage: (response) =>
    ...
    if response.message
      self.closeProjectLinksModal()
      $('.toast').messageToast.start(
        'Project link has successfully been updated',
        'success'
      )
      window.location.reload()
    else
      self.showFieldsError(response.errors)

  initializeEditOutputLinks: ->
    self = @
    self.bindProjectLinksModalToClose()
    self.bindProjectLinksModalToOpen()
    self.bindUpdateProjectLinksBtnToClick()
