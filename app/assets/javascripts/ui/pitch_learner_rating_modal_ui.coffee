class Pitch.LearnerRatingModal.UI
  ...
  initialiseModal: () =>
    ...
    $('.learner-pitch-tab-card, .rated-learner-message').off('click').click (e) ->
      self.learners_panel_id = $(this).children().closest(".persona-card-body, .panelist-card__content").attr('id')
      ...
      self.api.getLearnerRatings(self.learners_panel_id, self.flashErrorMessage)
        .then((data)->
          ratingsData
  ...
  populateRatingsModal: (ratingsData) =>
    ...
    $(".average-skills-rating").html(self.cumulativeAverage(self.totalratings.filter((v, i, a) => a.indexOf(v) == i)))

    if self.panelistView
      $('.panelist-comment').html(ratingsData[0].comment)
    else
      ratingsData.forEach((rating) =>
        if rating.learners_panel_id == parseInt(self.learners_panel_id)
  ...
  populateLfaModalDialog: (rating) ->
    ...
    <div class="group-3">
        ...
        <div class='flex-table'>
          ...
          <div class='rating-flex'>
            <ul>
              <li data-label="Cummulative Average"></li>
              <li class="lfa-rating-5">-</li>
            </ul>
          </div>
        </div>
      </div>
