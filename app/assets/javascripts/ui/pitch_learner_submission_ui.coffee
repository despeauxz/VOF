class Pitch.PitchLearnerSubmission.UI
  ...
  checkFormStatus: ->
    ...
    learnerRatings = { uiUx, apiFunctionality, errorHandling, projectUnderstanding,presentationalSkill, decision, comment, pitchId, panelId, learnerId }
  ...
  openRatingPreviewModal: (learnerRatings) ->
    ...
    $('#preview').html("<div class='rating_preview'>
      <table align='center' cellpadding='4rem'>
        ...
        <tr><td align='left'>Presentation Skill: </td> <td align='right' class='rating-value'> #{learnerRatings.presentationalSkill}</td></tr>
    ...
    $('#continue-rating-btn').off('click').click ->
      $('#continue-rating-btn').hide()
      $('.saving-btn').show()
      self.panel_api.submitLearnersRating(learnerRatings, self.flashErrorMessage)
        .then((data) -> (
          if data.message == "An error occurred"
            self.flashErrorMessage(data.message)
          else
            self.flashSuccessMessage(data.message)
            window.location.href = "#{location.protocol}//#{location.host}/#{pageUrl[1]}/#{data.pitch_id}/panels/#{data.panel_id}"
        ))
