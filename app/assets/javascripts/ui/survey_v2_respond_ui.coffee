class SurveyV2.Respond.UI
  ...
  getMultipleChoiceResponse: () ->
    ...
    $('.answer input[type="radio"]').off('click').on('click', ->
      ...
      if $(this).attr('name') == "option-#{question_id}"
        ...
        question_id = $(this).parents().eq(3).attr('id')
        answer = $(this).parent('.answer').find('label').text()
  ...
  getCheckboxResponse: ->
    ...
    $('.answer input[type="checkbox"]').each(->
      ...
      $(this).on 'click', ->
        ...
        answer_selected_id =  $(this).parent('.answer').find('label').text()
        if ($('input[type="checkbox"]').is(":checked"))
          if($.inArray(checkbox_id, response.checkbox_ids) != -1)
            ...
            response.answer_selected_ids.splice($.inArray(answer_selected_id, response.answer_selected_ids), 1);
          else
            response.checkbox_ids.push(checkbox_id)
            response.answer_selected_ids.push(answer_selected_id)
        else
          response.checkbox_ids.pop(checkbox_id);
          response.answer_selected_ids.pop(answer_selected_id);
  ...
  getDropdownResponse: ->
    ...
    $('select.survey-dropdown').on 'selectmenuchange', ->
      ...
      answer = $(".question-id-#{question_id}").attr('data-answer-id')
      dropdown_response = {
  ...
  getPictureOptionResponse: ->
    ...
    $('.option-image-list-item input[type="radio"]').off('click').on('click', ->
      ...
      $('select').selectmenu()
      question_id = $(this).parents().eq(3).attr('id')
      answer = $(this).parent('.option-image-list-item').find('label').text()
      question_type = $(this).parents().eq(3).attr('question_type')
      question_number = $(this).parents().eq(2).siblings().find('span').text()
  ...
  getPictureCheckboxResponse: ->
    ...
    $('.checkbox-image-list-item input[type="checkbox"]').each(->
      ...
      $(this).on 'click', ->
        ...
        answer_selected_id =  $(this).parent('.checkbox-image-list-item').find('label').text()
        if ($('input[type="checkbox"]').is(":checked"))  
          if($.inArray(picture_checkbox_id, picture_checkbox_response.picture_checkbox_ids) != -1) 
            ...
            picture_checkbox_response.answer_selected_ids.splice($.inArray(answer_selected_id, picture_checkbox_response.answer_selected_ids), 1);
          else
            picture_checkbox_response.picture_checkbox_ids.push(picture_checkbox_id)
            picture_checkbox_response.answer_selected_ids.push(answer_selected_id)
        else
          picture_checkbox_response.picture_checkbox_ids.pop(picture_checkbox_id);
          picture_checkbox_response.answer_selected_ids.pop(answer_selected_id);
  ...
  getDateResponse: (questions) ->
    ...
    dateInput.on 'click', ->
      ...
      calendar.on('change', ->
        ...
        answer = $(".data-date-#{question_id}").attr('data-answer-id')
  ...
  getTimeResponse: () ->
    ...
    $('.main-display').each(->
      ...
      answer = $(this).find(".data-time-answer-#{question_id}").text()
    ...
    paragraph.each(->
      $(document).ready(->
        paragraph.keyup debounce((->
          ...
          paragraph_answer = $(this).find('input').val()
          question_id = $(this).parents().eq(1).attr('id')
          question_type = $(this).parents().eq(1).attr('question_type')
          question_number = $(this).parents().eq(0).siblings().find('span').text()
          paragraph_response = {
  ...
  getScaleResponse: ->
    ...
    $('.no-padding-left li').on 'click', ->
      ...
      question_type = $(this).parents().eq(3).attr('question_type')
      question_number = $(this).parents().eq(2).siblings().find('span').text()
      answer = $(".data-scale-#{question_id}").attr('data-answer-id')
      scale_response = {
        value: scale_value,
        question_id: question_id,
        id: answer
  ...
  getMultiChoiceGridResponse: ->
    ...
    $('.radio-container input[type="radio"]').each(->
      $(this).on 'click', ->
        ...
        answer = $(this).parent('.table').find('label').text()
  ...
  getSurveyResponses: (surveys) ->
    self = @
    survey_response = self.survey_response #getting all responses
  ...
  populateQuestion: (surveys) ->
    ...
    if @totalSections == 1 && @editingDataAvailable == 0
      $('#next-preview').hide()
      $('.respond-submit-btn').removeClass('ui-helper-hidden')

    if @totalSections == 1 && @editingDataAvailable == 1
      $('#next-preview').hide()
      $('.respond-update-btn').removeClass('ui-helper-hidden')

    $('#prev-preview').hide()
    @renderSurveySection()
    @handlePreviewClick()
    @onSubmitResponse(surveys)
    @onSubmitResponseUpdate(surveys)
    @reorderNumbering()
  ...
  handlePreviewClick: ->
    $('#prev-preview').off('click').on('click', =>
       ...
       $(".respond-update-btn").addClass('ui-helper-hidden')
       if @currentSection == 1
         $('#prev-preview').hide()
    ...
    $('#next-preview').off('click').on('click', =>
      ++@currentSection
      $('#prev-preview').show()
      if @currentSection == @totalSections && @editingDataAvailable == 0
        $("#next-preview").hide()
        $(".respond-submit-btn").removeClass('ui-helper-hidden')
      if @currentSection == @totalSections && @editingDataAvailable == 1
        $("#next-preview").hide()
        $('.respond-update-btn').removeClass('ui-helper-hidden')
      $("#survey_section_#{@currentSection - 1}").addClass('ui-helper-hidden')
      @renderSurveySection(@currentSection)
  ...
  populateSelectQuestions: (survey_question, survey_sections) ->
    ...
    question_options.map (option) ->
        choices +=  "
            <option option-id=#{option.id} class='data-select-#{option.id} question-id-#{survey_question.id}' value='#{option.id}' question-id=#{survey_question.id} data-answer-id = '' >#{option.option}</option>
        "
    choices += "</select>"

  populateMultipleChoiceQuestions: (survey_question, survey_sections) ->
    question_options = survey_question.survey_options
    choices = "<div class='question-container'>"
    for option in question_options
      choices += 
        "<div class='answer'>
          <label style = 'display: none;' class ='data-options-answer-#{survey_question.id}'></label>
          <input type='radio' class='data-#{option.id}' name='option-#{survey_question.id}'>
          <span class='item-option' existing-answer option-id=#{option.id}>#{option.option}</span>
  ...
  populateCheckboxQuestions: (survey_question) ->
    ...
    @question_options.map (option) ->
      choices += 
        "<div class='answer'>
          <label style = 'display: none;' class ='checkbox-text-answer-#{option.id}'></label>
        ...
        table += 
          "<td>
            <label class='radio-container'>
              ...
              <label style = 'display: none;' class ='data-grid-options-answer-#{survey_question.id}'></label>
            </label>
          </td>"
      table += "</tr>"
      ...
      cols.forEach (col) =>
        table += "<td>
            <label class='checkmark-container'>
              <input type='checkbox' class='data-#{row.id}-#{col.id}' data-answer-id ='' data-answer-id = '' data-row-id=#{row.id} data-col-id=#{col.id} name='name-0'>
  ...
  populateScaleQuestions: (survey_question) ->
    ...
    while i <= max_value
      value += "<li class='data-scale-#{survey_question.id} data-scale-#{survey_question.id}-#{i}' data-answer-id =''>#{i}</li>"
      i++
        ...
        <div class='mdl-grid mdl-grid--no-spacing date'>
          ...
          <div class='mdl-cell mdl-cell--11-col mdl-cell--11-col-phone mdl-cell--11-col-tablet'>
            <div class='survey-item-container'>
              <input class='select-date cal data-date-#{survey_question.id}' data-answer-id =''  data-question_type='#{survey_question.type}' data-question_id='#{survey_question.id}' id='question-0-select-date' type='text' placeholder='Select Date'>
          ...
          <div class='mdl-cell mdl-cell--11-col mdl-cell--11-col-phone mdl-cell--11-col-tablet'>
            <div class='main-display'>
              <input class='display-time data-hour-#{survey_question.id}' type='number' placeholder='01:' size='2' maxlength='2' id='#{survey_question.id}'>
                ...
                <label style = 'display: none;' class ='data-time-answer-#{survey_question.id}'></label>
    ...
    choices = 
      "<div class='paragraph-wrapper' id='paragraph'>
        <div class='mdl-grid mdl-grid--no-spacing paragraph-txt'>
          ...
          <input style = 'display: none;' class='question-answer data-paragraph-answer-#{survey_question.id}' value=''>
        </div>
      </div>"
    ...
    @question_options.map (option) ->
      image_list +=
        "<li class='checkbox-image-list-item'>
          <label style = 'display: none;' class ='checkbox-pic-answer-#{option.id}'></label>
  ...
  onSubmitResponseUpdate: (surveys) ->
    self = @
    $('.respond-update-btn').on 'click', =>
      survey_responses = self.getSurveyResponses(surveys)
      return unless self.getRequiredQuestions(surveys, survey_responses) &&
        self.isRequiredOptionalQuestion(survey_responses)
      self.submitResponseUpdate(survey_responses, surveys.id
        (response) -> (
          self.flashErrorMessage "An error occured while updating")
        ).then((response) -> (
          self.flashSuccessMessage "Response Successfully updated"
          setTimeout (=>
              window.location.href = '/surveys-v2'
          ), 500
          ))
  ...
  onEditResponse: ->
    ...
    .then((response) -> (
      if response
        self.editingDataAvailable = 1
        self.survey_response["survey_responses_id"] = response.id
        $('.respond-submit-btn').hide()
        $('.respond-update-btn').removeClass('ui-helper-hidden')
        self.loader.show()
        setTimeout (=>
          self.configureResponse(response)
          self.loader.hide()
          if self.totalSections != 1
            $('.respond-update-btn').addClass('ui-helper-hidden')
        ), 1000
    ))
  ...
  configureResponse: (response) ->
    self = @
    answer_ids = []
    text_checkbox_answer_ids = []
    pic_checkbox_answer_ids = []
    multigrid_options_answer_ids = []
    if response.survey_grid_option_responses.length
      response.survey_grid_option_responses.map((key) ->
        ...
        if key.question_type == "SurveyMultigridCheckboxQuestion"
          $(".data-#{key.row_id}-#{key.col_id}").attr('data-answer-id', key.id)
          $(".data-#{key.row_id}-#{key.col_id}").click()
          answer_ids.push(key.id )
          self.survey_response["question_#{key.question_id}_data"] = answer_ids
        if key.question_type == "SurveyMultigridOptionQuestion"
          $(".data-grid-options-answer-#{key.question_id}").text(key.id)
          $(".data-#{key.row_id}-#{key.col_id}").click()
          multigrid_options_answer_ids.push(key.id )
          self.survey_response["question_#{key.question_id}_data_options"] = multigrid_options_answer_ids
    ...
    if response.survey_option_responses.length
      response.survey_option_responses.map((key) ->
        if key.question_type == "SurveySelectQuestion"
          $(".question-id-#{key.question_id}").attr('data-answer-id', key.id)
          $(".survey-dropdown").val(key.option_id).trigger('selectmenuchange')
          $(".survey-dropdown").selectmenu('refresh')
        if key.question_type == "SurveyMultipleChoiceQuestion"
          $(".data-#{key.option_id}").attr("checked", true).click()
          $(".data-options-answer-#{key.question_id}").text(key.id)
        if key.question_type == "SurveyPictureOptionQuestion"
          $(".data-options-answer-#{key.question_id}").text(key.id)
        if key.question_type == "SurveyPictureCheckboxQuestion"
          $(".checkbox-pic-answer-#{key.option_id}").text(key.id)
          pic_checkbox_answer_ids.push(key.id )
          self.survey_response["question_#{key.question_id}_data"] = pic_checkbox_answer_ids 
        if key.question_type == "SurveyCheckboxQuestion"
          $(".checkbox-text-answer-#{key.option_id}").text(key.id)
          text_checkbox_answer_ids.push(key.id )
          self.survey_response["question_#{key.question_id}_data"] = text_checkbox_answer_ids
    ...
    if response.survey_time_responses.length
      response.survey_time_responses.map((key) ->
        ...
        $(".data-time-answer-#{key.question_id}").text(key.id)
      )
    if response.survey_date_responses.length
      response.survey_date_responses.map((key) ->
        ...
        $(".data-date-#{key.question_id}").attr('data-answer-id', key.id)
    ...
    if response.survey_paragraph_responses.length
      response.survey_paragraph_responses.map((key) ->
        ...
        $(".data-paragraph-answer-#{key.question_id}").val(key.id)
      )
    if response.survey_scale_responses.length
      response.survey_scale_responses.map((key) ->
        ...
        $(".data-scale-#{key.question_id}").attr('data-answer-id', key.id)
      )
