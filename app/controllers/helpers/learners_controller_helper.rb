module LearnersControllerHelper
  ...
  def skip_empty_fields(projectlinks)
    updated_fields = {}
    %w(github_pages story_board challenge_one challenge_two challenge_three
       challenge_four heroku_one heroku_two).each do |field|
      link = projectlinks[field]
      link.empty? ? updated_fields : updated_fields[field] = link
    end
    updated_fields
  end

  def validate_website(websitelinks)
    errors = []
    websitelinks.each_key do |key|
      link = websitelinks[key]
      unless link.to_s.include? ".com"
        errors << key
      end
    end
    errors
  ...
  def validate_project_link_fields(params)
    errors = validate_website params
    errors
  end
  ...
  def get_specific_learner
    Bootcamper.find_by_email(session[:current_user_info][:email])
  end

  def update_project_output_links(updated_fields)
    new_fields = skip_empty_fields updated_fields
    errors = validate_project_link_fields new_fields
    errors.empty? ? get_specific_learner.update(new_fields) : errors
  end
end
