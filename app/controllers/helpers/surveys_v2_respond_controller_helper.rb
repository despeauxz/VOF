module SurveysV2RespondControllerHelper
  ...
  include SurveysV2RespondUpdateControllerHelper
  ...
  def build_survey_value_responses(survey_questions, response_data, res_id)
    get_response_details(survey_questions, response_data, res_id, "create")
  end

  def get_response_details(survey_questions, response_data, res_id, type)
    survey_questions.each do |survey_question|
      response = response_data["question_#{survey_question.id}"]
      next unless response

      save_or_update_response(response, res_id, type)
    end
  end

  def save_or_update_response(response, res_id, type)
    if type == "create"
      ...
      build_survey_scale_response_update(response, res_id)
      build_survey_paragraph_response_update(response, res_id)
      build_survey_date_response_update(response, res_id)
      build_survey_time_response_update(response, res_id)
    end
  end
