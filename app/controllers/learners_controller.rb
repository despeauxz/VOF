class LearnersController < ApplicationController
  ...
  def index
    @learner_links = get_specific_learner
    @learner = get_learner
  ...
  def update_learner_project_links
    project_links = params.permit(:github_pages, :story_board, :challenge_one,
                                  :challenge_two, :challenge_three,
                                  :challenge_four, :heroku_one, :heroku_two)
    status = update_project_output_links project_links
    response = status.is_a?(Array) ? { errors: status } : { message: "success" }
