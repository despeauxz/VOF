class SurveysV2RespondController < ApplicationController
  ...
  def update
    survey_id = parse(params[:survey_id])
    survey_data = parse(params[:survey_responses])
    survey_response_id = params[:survey_responses_id]
    new_response = get_survey_response_id(survey_response_id)
    build_responses_update(survey_id, new_response.id, survey_data)
    render json: { message: "Response Successfully updated" }, status: 201
  rescue SurveyException, JSON::ParserError => e
    Bugsnag.custom_notify(e)
    render json: e.message, status: 400
  end
